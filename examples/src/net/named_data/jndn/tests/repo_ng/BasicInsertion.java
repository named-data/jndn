...
public class BasicInsertion {
  ...
  requestInsert
    (Face face, Name repoCommandPrefix, Name fetchName,
     final SimpleCallback onInsertStarted, final SimpleCallback onFailed,
     long startBlockId, long endBlockId)
  {
    ...
    try {
      ...
      Interest interest = new Interest(new Name(repoCommandPrefix).append("insert")
        ...
        (interest,
         new OnData() {
           public void onData(Interest interest, Data data) {
             processRequestInsertResponse
               (data.getContent(), onInsertStarted, onFailed);
           }},
         new OnTimeout() {
  ...
  {
    requestInsert
      (face, repoCommandPrefix, fetchName, onInsertStarted, onFailed, -1, -1);
  }

  static void
  processRequestInsertResponse
    (Blob encodedResponse, final SimpleCallback onInsertStarted,
   ...
   * (endBlockId - startBlockId) + 1 packets. It might be simpler to finish when
   * onInterest has sent the packet for segment endBlockId, but there is no
   * guarantee that the interests will arrive in order. Therefore we send
   * packets until the total is sent.
   */
  static class ProduceSegments implements OnInterestCallback {
    ProduceSegments
      (KeyChain keyChain, Name certificateName, long startBlockId,
       long endBlockId, final SimpleCallback onFinished)
    {
      keyChain_ = keyChain;
       ...
       InterestFilter filter)
    {
      ...
      String content = "Data packet " + interest.getName().toUri();
    ...
    private final long startBlockId_;
    private final long endBlockId_;
    private final SimpleCallback onFinished_;
    private long nSegmentsSent_;
  }
  ...
  {
    try {
      Name repoCommandPrefix = new Name("/example/repo/1");
      Name repoDataPrefix = new Name("/example/data/1");

      long nowMilliseconds = System.currentTimeMillis();
      final Name fetchPrefix = new Name(repoDataPrefix).append("testinsert")
      ...
      ProduceSegments produceSegments = new ProduceSegments
