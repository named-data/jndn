 ...
 */
public class TestFullPSyncWithUsers {
  ...
  private static class Producer implements FullPSync2017WithUsers.OnUpdate {
    public Producer
      (Face face, KeyChain keyChain, Name syncPrefix, String userPrefixUri,
       int nUserPrefixes, int maxPublishedSequenceNo) throws IOException, SecurityException
    {
      face_ = face;
      ...
      for (int i = 0; i < nUserPrefixes_; ++i) {
        final Name prefix = new Name(userPrefixUri + "-" + i);
        ...
        face_.callLater
          (delay,
           new Runnable() {
             public void run() { doUpdate(prefix); }
           });
      }
    }

    private void
    doUpdate(final Name prefix)
    {
      fullPSync_.publishName(prefix);

      int sequenceNo = fullPSync_.getSequenceNo(prefix);
      System.out.println("Publish: " + prefix + "/" + sequenceNo);
    ...
    {
      for (PSyncMissingDataInfo update : updates) {
        for (int i = update.lowSequenceNo_; i <= update.highSequenceNo_; i++)
          System.out.println("Update " + update.prefix_ + "/" + i);
      }
    }

    private final Face face_;
    private final FullPSync2017WithUsers fullPSync_;
    private final int nUserPrefixes_;
    private final int maxPublishedSequenceNo_;
  ...
  main(String[] args) throws IOException
  {
    ...
    // Uncomment these lines to print FullPSync2017WithUsers debug messages.
