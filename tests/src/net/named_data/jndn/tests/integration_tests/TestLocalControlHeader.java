...
public class TestLocalControlHeader {
  Face face_;
  ...
  /**
   * Send the enable-local-control command, then send an interest and check
   * that the return data.getIncomingFaceId has a value.
   */
  @Test
  public void
  testIncomingFaceId() 
    throws SecurityException, IOException, EncodingException, InterruptedException
  {
    ...
    face_.expressInterest
      (makeLocalControlCommand(true, 1),
       new OnData() {
        public void onData(Interest interest, Data data) {
          processEnableLocalControlResponse(data, enabled);
        }},
       new OnTimeout() {
        public void onTimeout(Interest interest) {
          enabled[0] = false;
          System.out.println("enable-local-control command timed out.");
        }});
  ...
  /**
   * Process the response from the enable-local-control command, and send an
   * interest to fetch any data packet to check for IncomingFaceId.
   * @param data The response Data packet.
   * @param enabled On success or error, set enabled[0] = false;
   */
  private void
  processEnableLocalControlResponse(Data data, final boolean[] enabled)
  {
    try {
      face_.expressInterest
       (new Name(),
        new OnData() {
          public void onData(Interest interest, Data data) {
            // We got a data packet. Check that it is not unspecified (-1).
            assertTrue("Data packet does not have an IncomingFaceId",
                       data.getIncomingFaceId() >= 0);
            enabled[0] = false;
          }},
        new OnTimeout() {
          public void onTimeout(Interest interest) {
            enabled[0] = false;
            System.out.println("Fetch any data timed out.");
          }});
    } catch (IOException ex) {
      enabled[0] = false;
      fail("I/O error while fetching data " + ex);
    }
  }

  /**
   * Make and sign a command interest to enable or disable LocalControlHeader.
   * @param enable If true, use "enable-local-control". Otherwise, use
   * "disable-local-control".
   * @param localControlFeature The value for the ControlParameters as follows:
   * 1 = IncomingFaceId, 2 = NextHopFaceId, 3 = CachingPolicy.
   * @return
   */
  private Interest
  makeLocalControlCommand(boolean enable, int localControlFeature)
